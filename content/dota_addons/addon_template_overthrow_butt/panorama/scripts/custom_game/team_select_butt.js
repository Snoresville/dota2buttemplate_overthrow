...
var BSettings = [
    $.GetContextPanel().FindChildInLayoutFile("BSettingsColumn1"),
    $.GetContextPanel().FindChildInLayoutFile("BSettingsColumn2"),
];
var path_links = {};
var clean = false;
...
function generateCategory(odata, category_id) {
    var data = odata.OBJ;
    if (undefined == data || typeof data != "object" || data.length == 0)
        return;
    let no_objects = true;
    for (let i in data) {
    ...
    if (undefined == data.NAME) {
        category.FindChildTraverse("SettingCategoryText").text = $.Localize(
            "#Butting_" + category_id
        );
    } else {
    ...
    category_block
        .FindChildTraverse("SettingCategoryBlock_Tittle")
        .SetAttributeInt("order", -1000);
    if (undefined == data.NAME) {
        category_block.FindChildTraverse(
            "SettingCategoryBlock_TittleText"
        ).text = $.Localize("#Butting_" + category_id);
    } else {
        category_block.FindChildTraverse(
            "SettingCategoryBlock_TittleText"
        ).text = data.NAME;
	...
	*/
    ...
    if (undefined !== data.TYPE && data.TYPE == "OPTIONAL") {
        ...
        setting.FindChildTraverse("SettingValue").text =
            $.Localize("#Butting_ENABLED");
...
function generateSetting_options(
    data,
    category_block,
    category_id,
    setting_id
    ...
    var setting = $.CreatePanel("Panel", category_block, "");
    if (undefined == data.ORDER) setting.SetAttributeInt("order", 1000);
    else setting.SetAttributeInt("order", data.ORDER);
...
function generateSetting_number(data, category_block, category_id, setting_id) {
    ...
    if (undefined == data.NAME) {
        setting.FindChildTraverse("SettingNumberText").text = $.Localize(
            "#Butting_" + category_id + "_Option_" + setting_id
        );
    } else {
        setting.FindChildTraverse("SettingNumberText").text = data.NAME;
    }
    setting
        .FindChildTraverse("SettingValue")
        .SetAttributeInt("decimal", data.DECIMAL);
    if (undefined !== data.UNIT) {
        setting
            .FindChildTraverse("SettingValue")
            .SetAttributeString("unit", data.UNIT);
        setting.FindChildTraverse("SettingValue").text =
            data.VALUE.toFixed(data.DECIMAL) + data.UNIT;
    } else {
        setting.FindChildTraverse("SettingValue").text = data.VALUE.toFixed(
            data.DECIMAL
        );
    }
    setting
        .FindChildTraverse("SettingValue")
        .SetPanelEvent("onfocus", function () {
            if (!IsHost) {
                return;
            }
...
function generateSetting_boolean(
    data,
    category_block,
    category_id,
    setting_id
    ...
    setting.BLoadLayoutSnippet("SettingBoolean");
    if (undefined == data.NAME) {
        setting.FindChildTraverse("SettingValue").text = $.Localize(
            "#Butting_" + category_id + "_Option_" + setting_id
        );
    } else {
    ...
    if (optype == 0) {
        ...
        panel
            ...
            .SetPanelEvent("oninputsubmit", function () {
                ...
                let unit = panel
                    .FindChildTraverse("SettingValue")
                    .GetAttributeString("unit", "");
                let min = panel
                    .FindChildTraverse("SettingValue")
                    .GetAttributeInt("min", -99999);
                let max = panel
                    .FindChildTraverse("SettingValue")
                    .GetAttributeInt("max", 99999);
                if (val < min) {
                    val = min;
                }
                if (val > max) {
        ...
        panel
            ...
            .SetPanelEvent("onblur", function () {
                ...
                panel.FindChildTraverse("SettingValue").text =
                    val.toFixed(
                        panel
                            .FindChildTraverse("SettingValue")
                            .GetAttributeInt("decinal", 0)
                    ) + unit;
            });
    }
}
...
function SortElements(elements, changes) {
    ...
    if (changes) {
        $.Schedule(0.01, function () {
            SortElements(parent.Children(), changes);
        });
    }
}
        ...
        if (valuePanel) {
            ...
            switch (true) {
                ...
                case "TextEntry" === panelType:
                    valuePanel.text =
                        val.toFixed(valuePanel.GetAttributeInt("decimal", 0)) +
                        valuePanel.GetAttributeString("unit", "");
                    break;
                default:
